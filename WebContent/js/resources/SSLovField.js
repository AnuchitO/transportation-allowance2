...
Ext.ss.form.LovField = Ext.extend(Ext.form.TriggerField, {
	...
	setValue: function (v, untriging) {
		if (untriging)
		{
			...
			Ext.form.TriggerField.superclass.setValue.call(this, v);
		}
		else
		{
	...
	initLov: function () {
		...
		if (this.windowLov === undefined) {
			...
			if(!this.customColumns) {
				...
				Ext.each(this.customColumns, function(col, index) {
					Ext.apply(col, {menuDisabled: true});
					if(!col.hasOwnProperty('width')){
						Ext.apply(col, {width: 30});
					}
					if(!col.hasOwnProperty('align')){
						Ext.apply(col, {align: 'left'});
					}
					Ext.apply(col, {header: '<center>' + col.header + '<center>'});
				});
				this.columns = Ext.value(this.columns, this.customColumns);
			}
